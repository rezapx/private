...
useradd -M -s /bin/false 08983512220
echo "08983512220:$PASS" | chpasswd
echo "08983512220" > pass.txt
echo "$PASS" >> pass.txt
...
# install squid
yum -y install squid
wget -O /etc/squid/squid.conf "http://internetku.net/files/autoscript/conf/squid-centos.conf"
...
echo "---------------"  | tee -a log-install.txt
echo "User     : 08983512220"  | tee -a log-install.txt
