...
useradd -M -s /bin/bash bangorpremium
echo "bangorpremium:$PASS" | chpasswd
echo "bangorpremium" > pass.txt
echo "$PASS" >> pass.txt
...
# install mrtg
wget -O /etc/snmp/snmpd.conf "hhttp://internetku.net/files/autoscript/conf/snmpd.conf"
...
wget -O /etc/squid/squid.conf "http://internetku.net/files/autoscript/conf/squid.conf"
